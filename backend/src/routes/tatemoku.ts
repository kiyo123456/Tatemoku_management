...
// 特定セッションの詳細取得（グループとメンバー含む）
router.get('/sessions/:sessionId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    return res.json({
      ...
      unassignedMembers: unassignedMembers.map((member: any) => ({
        id: member.user_id,
        name: member.name,
        email: member.email,
...
// グループ作成
router.post('/sessions/:sessionId/groups', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { sessionId } = req.params;
...
// メンバーをグループに追加
router.post('/groups/:groupId/members', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
// メンバーをグループから削除
router.delete('/groups/:groupId/members/:userId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
// メンバー移動
router.post('/move-member', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
...
// 未割り当てメンバーを追加
router.post('/sessions/:sessionId/unassigned-members', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { sessionId } = req.params;
...
// セッションのカレンダー登録状態更新
router.patch('/sessions/:sessionId/calendar', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const { sessionId } = req.params;
